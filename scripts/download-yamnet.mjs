...
async function main() {
  console.log("Preparing YAMNet model...");
  ...
  try {
    ...
    for (const url of MODEL_ARCHIVES) {
      try {
        ...
        console.warn(
          `Model download failed from: ${url}`
        );
      }
    }
    ...
    console.log("");
    console.log(
      "YAMNet model prepared successfully!"
    );

    console.log(
      `Output directory: ${publicModel}`
    );
